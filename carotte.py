...
try:
    ...
    print(
        "Warning: Install module 'colored_traceback' for better tracebacks",
        file=sys.stderr,
    )
...
try:
    ...
    assignhooks.transformer.AssignTransformer.visit_Assign = (
        alt_transformer.visit_Assign
    )
    import alt_instrument
except ModuleNotFoundError:
    print(
        "Warning: Install module 'assignhooks' for better variable names",
        file=sys.stderr,
    )
    assignhooks = None
...
if sys.version_info < MIN_PYTHON:
    print(
        "Python %s.%s or later is required" % MIN_PYTHON, file=sys.stderr
...
def process(
    ...
    if assignhooks is not None:
        if assignhooks_path is None:
            ...
            alt_instrument.path = [
                os.path.abspath(i) for i in assignhooks_path.split(":")
            ]
...
def main() -> None:
    ...
    parser.add_argument(
        "-p",
        "--assignhooks-path",
        help="Python files from these folder will have nice variable names if `assignhooks` is present. Takes a colon separated list of paths",
    )
    parser.add_argument(
        "-m",
        "--module-name",
        help="Carotte.py module entrypoint. Usually, you don't have to specify it unless you specify --python-path option. Default: module_file filename without `.py`",
    )
    parser.add_argument(
        "-y",
        "--python-path",
        help="Additionnal paths python will look to while importing modules (the entrypoint is a module). Default: directory containing module_file",
    )
    args = parser.parse_args()
    process(
        args.module_file[0],
        args.output_file,
        args.assignhooks_path,
        args.module_name,
        args.python_path,
    )
